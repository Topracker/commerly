...
export default function Fiado() {
  ...
  async function carregar() {
    ...
    setLoja(lojaData)
    const { data } = await supabase.from('fiado').select('*').eq('loja_id', lojaData.id).order('created_at', { ascending: false })
    setFiados(data || [])
  }

  async function salvar() {
    if (!clienteNome || !descricao || !valor) return alert('Preencha todos os campos!')
    setLoading(true)
  ...
  async function marcarPago(id: string, pago: boolean) {
    await supabase.from('fiado').update({ pago: !pago }).eq('id', id)
    carregar()
  }

  async function remover(id: string) {
    if (!confirm('Remover fiado?')) return
    await supabase.from('fiado').delete().eq('id', id)
    carregar()
  }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <h1 className="text-3xl font-bold text-white">Fiado</h1>
          </div>
            ...
            {filtrados.map(f => (
              <div key={f.id} className={`bg-gray-900 rounded-2xl p-4 flex items-center justify-between gap-4 ${f.pago ? 'opacity-60' : ''}`}>
                ...
                <div className="flex gap-2 shrink-0">
                  ...
                  <button onClick={() => remover(f.id)} className="bg-red-900 hover:bg-red-800 text-red-300 px-3 py-1.5 rounded-lg text-sm transition">
                    Remover
                  </button>
                </div>
              </div>
