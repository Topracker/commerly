...
export default function ClienteLogin() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (!session) return
      const { data } = await supabase.from('clientes').select('id').eq('user_id', session.user.id).single()
  ...
  async function entrar() {
    ...
    const { data } = await supabase.from('clientes').select('id').eq('user_id', user!.id).single()
    router.push(data ? '/cliente/buscar' : '/cliente/onboarding')
  }
    ...
    <main className="min-h-screen bg-gray-950 flex items-center justify-center p-6">
      <div className="bg-gray-900 rounded-3xl p-8 w-full max-w-sm">
        <div className="mb-6">
          ...
          <p className="text-gray-400">Descubra comércios locais</p>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <input
            ...
            className="bg-gray-800 text-white rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-green-500"
          />
          <button onClick={entrar} disabled={loading} className="bg-green-600 hover:bg-green-700 text-white font-semibold py-3 rounded-xl transition disabled:opacity-50">
