...
export default function ClienteLogin() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const outro = await outroPapel(supabase, userId, 'cliente')
      if (outro) {
      ...
      <div className="bg-gray-900 rounded-3xl p-8 w-full max-w-sm">
        ...
        {tela === 'escolha' && (
          <div className="flex flex-col gap-3">
            ...
            <BotaoGoogle voltar="/cliente/login" onErro={setErro} />
        ...
        {tela === 'login-senha' && (
          <div className="flex flex-col gap-4">
            ...
            <button onClick={() => router.push('/recuperar-senha?voltar=/cliente/login')}
          ...
          <div className="flex flex-col gap-4">
            {senhaRecusada ? (
              <>
                ...
                <CampoSenha id="retry-cliente" value={senha} onChange={setSenha} onEnter={salvarSenhaRecusada}
                  placeholder="Nova senha *" className={inp} />
                <button onClick={salvarSenhaRecusada} disabled={loading || !senhaValida(senha)} className="bg-green-600 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-xl transition">
