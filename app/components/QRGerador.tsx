...
// Gera QR code para um ou mais destinos, com download em PNG, SVG e PDF.
export function QRGerador({ destinos, nomeArquivo = 'commerly-qr', tituloPdf }: { destinos: DestinoQR[]; nomeArquivo?: string; tituloPdf?: string }) {
  ...
  // PDF de verdade: o jsPDF (~350 KB) só entra no bundle de quem clica.
  async function baixarPdf() {
    if (!png) return
    const { baixarQrPdf } = await import('../lib/qrPdf')
    baixarQrPdf({
      png,
      titulo: tituloPdf || destino.label,
      legenda: destino.label,
      url: destino.url,
      nomeArquivo: `${nomeArquivo}-${destino.chave}`,
    })
  }
    ...
    <div className="bg-card border border-borda rounded-2xl p-5">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-5">
        ...
        <div className="flex-1 w-full">
          ...
          <p className="text-gray-500 text-xs mt-2 flex items-center gap-1"><Download size={12} /> Baixe, imprima e cole na loja. O PDF já vem pronto para A4.</p>
        </div>
      </div>
    </div>
  )
