-- ============================================================================
-- GUARD DE PEDIDOS: pedido PAGO leva a taxa que a Stripe cobrou
-- (auditoria 2026-09-11, achado V2)
-- ----------------------------------------------------------------------------
-- No horário de pico (sex/sáb/dom 18h–22h) o guard recalculava a taxa de
-- entrega com +30% no INSERT, mesmo para o pedido que o webhook da Stripe
-- materializa a partir de `pedidos_pendentes` — ou seja, DEPOIS de o cliente
-- já ter pago. O checkout cobrava X, o banco gravava X×1,3 em `taxa_entrega`,
-- `valor_corrida` e `total`, e o repasse ao entregador (confirmar-entrega)
-- saía 30% maior do que a plataforma reteve.
--
-- Corrigir só o checkout não bastava: o Pix tem 30 min de QR, então checkout
-- às 17:59 e pagamento às 18:05 (ou 21:58 → 22:03) divergiriam do mesmo jeito.
-- Para um pedido pago, a fonte da verdade é o que foi cobrado.
--
-- MUDANÇA (única): no INSERT sem festa, quando o chamador é o service_role e
-- o pedido traz `stripe_session_id` (= veio do webhook, já pago), a taxa que
-- chega do pendente é mantida em vez de recalculada. Pedido pago NA ENTREGA
-- (insert do cliente, auth.role() = 'authenticated') continua exatamente como
-- antes: taxa por distância + surge de pico calculados aqui.
--
-- Mesma condição que `bloquear_pedido_loja_sem_plano` já usa. Um cliente
-- inserindo pela chave anon tem role 'authenticated' e não passa por aqui.
--
-- Corpo abaixo = corpo VIVO lido do banco em 2026-09-11 (md5
-- 2b16dc3138056cf010120b4ae5e8aa84) + apenas o bloco "taxa" alterado.
-- Testado em transação com rollback antes de aplicar (service_role x
-- authenticated, em horário de pico).
--
...
-- Conferência (deve conter 'service_role' e 'stripe_session_id' no bloco da taxa):
